...
const FinancialForm = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await api.post('/document/extract', uploadData, {
        headers: {
      ...
      if (response.data.success && response.data.data) {
        ...
        setFormData(prev => ({
          ...
          income: extracted.income?.toString() || prev.income,
          loanAmount: extracted.loanAmount?.toString() || prev.loanAmount,
          numBankAccounts: extracted.numBankAccounts?.toString() || prev.numBankAccounts,
          numCreditCards: extracted.numCreditCards?.toString() || prev.numCreditCards,
          numOfDelayedPayment: extracted.numOfDelayedPayment?.toString() || prev.numOfDelayedPayment,
        }));
      }
    } catch (err: unknown) {
      console.error('Upload error:', err);
      let errorMessage = 'Failed to extract document. Please fill manually.';
      if (err && typeof err === 'object' && 'response' in err) {
        const res = (err as any).response;
        errorMessage = res?.data?.error || res?.data?.details || errorMessage;
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const numericalFields: Array<keyof typeof formData> = ['age', 'income', 'existingDebtPayments', 'loanAmount', 'loanRate', 'loanTerm', 'numBankAccounts', 'numCreditCards', 'numOfDelayedPayment'];
    ...
    if (hasNegativeValues) {
      setError('Please enter positive values for all numerical fields');
      return;
    }

    // Validate required fields
    if (!formData.name || !formData.age || !formData.income || !formData.loanAmount || 
        !formData.loanRate || !formData.loanTerm || !formData.loanPurpose) {
    ...
    try {
      ...
      const response = await api.post('/loans', {
        ...
        numBankAccounts: parseFloat(formData.numBankAccounts || '0'),
        numCreditCards: parseFloat(formData.numCreditCards || '0'),
        numOfDelayedPayment: parseFloat(formData.numOfDelayedPayment || '0'),
        creditMix: formData.creditMix
      });

      // Store application data in sessionStorage to pass to output page
      sessionStorage.setItem('loanApplication', JSON.stringify(response.data));
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-[url('/api/placeholder/1920/1080')] bg-cover bg-center before:content-[''] before:absolute before:inset-0 before:bg-[var(--color-text)] before:backdrop-blur-sm">
      ...
      <div className="w-full max-w-2xl bg-white/95 rounded-3xl shadow-2xl overflow-hidden z-10 transition-all duration-300 border border-white/30 hover:shadow-blue-500/20 backdrop-blur-md">
        <div className="p-8">
          ...
          <div className="mb-8 p-6 bg-blue-50/50 border-2 border-dashed border-blue-200 rounded-xl text-center transition-all hover:bg-blue-50 hover:border-blue-300">
            <h3 className="text-lg font-semibold text-blue-900 mb-2">Optional: Auto-fill from Document</h3>
            <p className="text-sm text-gray-600 mb-4">Save time by uploading a bank statement or payslip (PDF). Your document is processed instantly in memory and never stored.</p>
            <div className="relative">
              ...
              />
              <button disabled={isUploading} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-medium shadow-sm hover:bg-blue-700 transition-colors disabled:opacity-50">
                {isUploading ? 'Extracting Data...' : 'Select PDF Document'}
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div className="group">
                <label htmlFor="numBankAccounts" className="block text-sm font-medium text-gray-700 mb-1 group-focus-within:text-blue-600 transition-colors">
              ...
              <div className="group">
                <label htmlFor="numCreditCards" className="block text-sm font-medium text-gray-700 mb-1 group-focus-within:text-blue-600 transition-colors">
              ...
              <div className="group">
                <label htmlFor="numOfDelayedPayment" className="block text-sm font-medium text-gray-700 mb-1 group-focus-within:text-blue-600 transition-colors">
                  Number of Delayed Payments
                </label>
                <input
                  type="number"
                  name="numOfDelayedPayment"
                  id="numOfDelayedPayment"
                  min="0"
                  value={formData.numOfDelayedPayment}
                  onChange={handleChange}
                  className="w-full px-4 py-3 text-base bg-white/80 text-gray-800 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 hover:border-blue-400"
                  required
                />
              ...
              <div className="group">
                <label htmlFor="creditMix" className="block text-sm font-medium text-gray-700 mb-1 group-focus-within:text-blue-600 transition-colors">
                  Credit Mix
                </label>
                <div className="relative">
                  <select
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                className="px-6 py-2 mr-4 bg-white text-blue-600 font-medium rounded-lg border border-blue-200 hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Reset
              </button>
              <button
                type="submit"
                disabled={isSubmitting}
                className="px-8 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 shadow-lg hover:shadow-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
              >
